...
while robot.step(TIME_STEP) != -1:
    ...
    if contadorReset > reset + 1000:
        if (geracao > 100):
            ...
            indice = list_media.index(melhor_individuo)
            print(f"Melhor Individuo: {list_pontos[indice]}, Indice: {indice}")
            quit()
        ...
        for x in range(len(list_error)):
            somatoria = somatoria + list_error[x]
            #print(f"X: {list_error[x]}")
        print(f"Somatória: {somatoria}")
        media = abs(somatoria / len(list_error))
